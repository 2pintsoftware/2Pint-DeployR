...
# Function to get the latest .NET 10 version by checking URLs starting from 10.0.0
function Get-LatestDotNet10Version {
    ...
    $baseVersion = "10.0.0"  # Starting point
    $latestVersion = $baseVersion
    $maxAttempts = 100  # Limit to avoid infinite loop

    Write-Host "Searching for the latest .NET 10 version starting from $baseVersion..."
...
$latestVersion = Get-LatestDotNet10Version
...
$tempDir = "$env:TEMP\.NET10Installers"
