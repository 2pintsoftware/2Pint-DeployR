# PowerShell script to download and install the latest PowerShell 7.6.x on Windows

# Function to check for the latest PowerShell 7.6.x release
...
function Install-PowerShell76X {
    ...
    function Get-LatestRelease {
        ...
        $latest = $releases | Where-Object { $_.tag_name -match '^v7\.6\.\d+$' } | Select-Object -First 1
    ...
    if (-not $version) {
        Write-Host "Could not find a PowerShell 7.6.x release."
        exit 0
    }
