...
These steps are two parts, a JSON file which gets imported into DeployR as shown below, and a PowerShell script which you'll need to create a content item to include them in.

The step defintion(s), once imported, need to be updated with the content item that was created.
...
- Run the Import Command to import the json file that was downloaded.
...
Grab the script(s) associted with the custom step definition and create a DeployR content item.  

![Image01](./media/stepdef01.png)

### Update Step Definition to use the Content Item

Once the step definition is imported and content item created, set the content item with the scripts in the step definition.
